...
const FlightTableRow = ({ flight, handleMouseEnter, handleMouseLeave, showCompany = false }) => (
  ...
  >
    ...
    <td className="py-2 px-3">
      {isWithin24Hours(flight.createdDate) && (
...
const CompanyDetails = ({ company, sortDirection, filters, setHoveredFlightCoords }) => {
  ...
  const rankOverall = company?.rankOverall || null;
  
  // Handle mouse enter/leave events directly
...
const DateAdjustment = ({ 
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (isTabContent && !dataFetched && isMountedRef.current && adjustment.ids && adjustment.ids.length > 0) {
        try {
          ...
          console.error('Error fetching date adjustment data:', err);
        }
      }
    };
  ...
  }, [isTabContent, adjustment, getCompaniesByCategory, dataFetched]);
      ...
      if (company.haves && Array.isArray(company.haves)) {
        company.haves.forEach(have => {
          flights.push({
            ...
            rankOverall: company.rankOverall
          });
        });
      }
    });
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-lg text-black">{adjustment.title || 'Flexi Legs'}</h3>
