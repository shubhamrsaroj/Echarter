...
export const PipelineProvider = ({ children }) => {
  ...
  const [selectedItineraryDetails, setSelectedItineraryDetails] =
    useState(null);
  ...
  const addItinerary = async (itineraryData) => {
    try {
      const response = await SellerMarketService.addItineraryService(
        itineraryData
      );
  ...
  const updateItinerary = async (itineraryData) => {
    try {
      const response = await SellerMarketService.updateItineraryService(
        itineraryData
      );
  ...
  const getOptionsbyItineraryId = useCallback(async (itineraryId) => {
    ...
    return new Promise((resolve) => {
      requestTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          const response =
            await SellerMarketService.getOptionsbyItineraryIdService(
              itineraryId
            );

          if (response.success && response.data) {
            const flights =
              response.data.itineraryResponseNewdata?.itinerary?.map((leg) => ({
            ...
            const error = {
              message:
                ...
                response.message ||
                "No options found for this itinerary",
          ...
          const errorMessage =
            error.message || "An error occurred while fetching options";
  ...
  const getUserItineraries = useCallback(async (days = 7) => {
    ...
    try {
      ...
      const response = await SellerMarketService.getUserItineraries(
        userData.id,
        days
      );
  ...
  const getItineraryById = useCallback(async (id) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        throw new Error(
          response.message || "Failed to fetch itinerary details"
        );
      }
    } catch (err) {
      setItineraryDetailsError(
        err.message || "Error fetching itinerary details"
      );
      console.error(err);
      return null;
    } finally {
  ...
  const getReviews = useCallback(async (id) => {
    try {
      ...
      if (response && response.data) {
        setSelectedReviews(response.data); // ✅ Only set the actual reviews
        return response.data;
      } else {
