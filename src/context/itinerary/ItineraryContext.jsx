    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.data && typeof response.data === 'object') {
          // Structured error (like in second image)
          setError(response.data);
        } else {
          // Simple error message (like in first image)
          setError({
            message: response.message || 'No found any Itinerary',
      ...
      setError({
        message: error.message || 'An error occurred while fetching itinerary'
      });
      throw error;
    } finally {
      setLoading(false);
